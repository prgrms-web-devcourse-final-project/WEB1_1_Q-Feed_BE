...
public class UserInterestRepositoryImpl implements UserInterestRepository {
    ...
    public List<UserInterest> findByUserId(UserId userId) {
        ...
        List<UserInterestEntity> entities = queryFactory.selectFrom(userInterestEntity)
                .where(userInterestEntity.userId.eq(userId.getValue()))
                .fetch();
    ...
    @Override
    public void save(List<UserInterest> userInterests) {
                // CategoryEntity 리스트 조회
        List<Long> categoryIds = userInterests.stream()
                .map(i -> i.getCategoryId().getValue())
                .toList();


        // UserInterestEntity 리스트 생성
        List<UserInterestEntity> newEntities = mapper.toEntityList(userInterests);
    ...
    public List<String> findUserIdsByInterestCategories(List<Long> categoryIds, String userId) {
        return queryFactory
                .select(userInterestEntity.userId)
